...
int main(int argc, char** argv) {
	...
	const string& model_file = "\\caffe-ssd-microsoft-gpu\\data\\SSD_300x300\\deploy.prototxt";
	const string& weights_file = "\\caffe-ssd-microsoft-gpu\\data\\SSD_300x300\\VGG_VOC0712_SSD_300x300_iter_120000.caffemodel";

	const string& mean_file = FLAGS_mean_file;
	const string& mean_value = FLAGS_mean_value;
	const string& file_type = FLAGS_file_type;
	const string& out_file = "\\caffe-ssd-microsof-gput\\data\\image.txt";
	...
	string file = "\\caffe-ssd-microsoft-gpu\\data\\SSD_300x300\\000017.jpg";
	...
	while (1)
	{
		if (file_type == "image") {
			...
			cv::Mat img;
			int frame_count = 0;
			VideoCapture imgs("\\caffe-ssd-microsoft-gpu\\data\\001.avi");
			...
			while (true) {
				...
				if (!success) {
					LOG(INFO) << "Process " << frame_count << " frames from " << imgs.read(img);
					break;
				}
